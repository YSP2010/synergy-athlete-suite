...
function RaceDetail() {
  ...
  const preset =
    race.race_type !== "custom" ? RACE_PRESETS[race.race_type as keyof typeof RACE_PRESETS] : null;
  const distances = {
  ...
  return (
    <div className="space-y-5 pb-10">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Badge variant={race.priority === "A" ? "default" : "outline"}>
          Priorität {race.priority}
        </Badge>
      </div>
      ...
      <div className="card-elevated space-y-3 p-5">
        ...
        <p className="text-xs text-muted-foreground">
          Format h:mm:ss oder mm:ss. Leere Felder werden verteilt.
        </p>
        <div className="grid gap-3 sm:grid-cols-3">
          <Field
            label="Gesamt"
            value={goal.total}
            onChange={(v) => setGoal({ ...goal, total: v })}
          />
          {distances.swimM > 0 && (
            <Field
              label="Schwimmen"
              value={goal.swim}
              onChange={(v) => setGoal({ ...goal, swim: v })}
            />
          )}
          {distances.swimM > 0 && (
            <Field
              label="Wechsel 1"
              value={goal.t1}
              onChange={(v) => setGoal({ ...goal, t1: v })}
            />
          )}
          {distances.bikeM > 0 && (
            <Field label="Rad" value={goal.bike} onChange={(v) => setGoal({ ...goal, bike: v })} />
          )}
          {distances.bikeM > 0 && (
            <Field
              label="Wechsel 2"
              value={goal.t2}
              onChange={(v) => setGoal({ ...goal, t2: v })}
            />
          )}
      ...
      <div className="card-elevated space-y-3 p-5">
        ...
        {!legs.length ? (
          <p className="text-sm text-muted-foreground">
            Trage eine Gesamt-Zielzeit ein, dann rechnen wir die Aufteilung.
          </p>
        ) : (
          <div className="divide-y divide-border">
            {legs.map((l) => (
              <div key={l.key} className="flex items-center gap-3 py-2.5">
                ...
                <span className="w-16 text-right font-display text-sm font-semibold">
                  {fmtDuration(l.timeS)}
                </span>
          ...
          {taper.map((t) => (
            <div key={t.weeksToRace} className="rounded-lg bg-elevated p-3 text-sm">
              <div className="font-medium">
                {t.weeksToRace} Woche{t.weeksToRace > 1 ? "n" : ""} vorher · Umfang{" "}
                {Math.round(t.volumeFactor * 100)} %
              </div>
              <div className="text-muted-foreground">{t.hint}</div>
            </div>
          ))}
          <p className="text-xs text-muted-foreground">
            Vorschlag, kein Zwang – dein Wochenplan bleibt unverändert.
          </p>
        </div>
      )}
